...
// Init mike stuff  
    typedef itk::FixedArray<double, ImageDimension> singleEigenVectorArrayType; // a vector (3,1)
    typedef itk::Image<singleEigenVectorArrayType, ImageDimension> MaximaVectorType;   // a 3D image of vector(3,1)     
    typedef MaximaVectorType mikeMaximaImage;   // The image for mike
        
    mikeMaximaImage = MaximaVectorType::New(); // Allocate it.
    
    typename MaximaVectorType::OutputImageType::Pointer MikeMaximaVectorOutputImage;   // Create a pointer to access it.   
    MikeMaximaVectorOutputImage = mikeMaximaImage->GetOutput();  // Point on the output buffer.    
    
    typedef itk::ImageRegionIterator<MikeMaximaVectorOutputImage> MaximaVectorIteratorType; // Create the iterator.  
    MaximaVectorIteratorType itMaxima(MikeMaximaVectorOutputImage, m_DiffusionTensorImage->GetLargestPossibleRegion()); // Create a iterator with same regions as the diffusion tensor.
    itMaxima.GoToBegin();
    ...
    typename MaximaVectorType::PixelType eigenVectorArray; // Create a vector (3,1) to retreive the data for mike.
    ...
    for (unsigned int i = 0; i < ImageDimension; ++i)
    {
      ...
      // Refering to : https://itk.org/Doxygen/html/classitk_1_1SymmetricEigenAnalysis.html#a0aa8cac66965c6a1c340ca3413629ebe
      ...
      // Note that into : itkSymmetricEigenVectorAnalysisImageFilter.h, the order is set as lambda3 is the biggest, so we are supposed to take tensor v3, which is tensor(:, 2) 
      eigenVectorArray(i) = tensor(i, 2); // MIKE: Im not fully sure of that, but lets start with this. 
    }

    itDiff.Set(tensor);
    itMaxima.Set(eigenVectorArray);    
  ...
  // Check if we are at the latest tensor iteration (which is iteration number - 2, because - 1 is the last and it for saving the vesselness (check above).
   if(this->GetNumberOfIterations() - 2 == this->GetElapsedIterations())
   {
    ...
    typedef itk::ImageFileWriter<MaximaVectorType> ImageWriterType;
    ImageWriterType::Pointer writer = ImageWriterType::New();
    writer->SetFileName("mike_maxima_things.nii.gz");
    writer->SetInput(mikeMaximaImage->GetOutput());
